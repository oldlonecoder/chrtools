...
//static  chattr::format color_format = chattr::format::ansi256;
// ----------------------color -----------------------
...
//    if (C == color::Reset) { return "</span>"; }
    ...
    Out << "#"
        << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(D.R)
        << std::setw(2) << std::setfill('0') << static_cast<int>(D.G)
        << std::setw(2) << std::setfill('0') << static_cast<int>(D.B);

    return Out.str();
}
/**
 * @brief Generate the html background color attributes sequnce.
 *
 * @param C
 * @return std::string
 *
 ...
 * @return chattr::pair&
 */
chattr::pair &chattr::pair::operator>>(std::string &out)
{
    out = chattr::ansi(*this);
    return *this;
}
 ...
 * @return std::string
 */
std::string chattr::ansi(chattr::pair color_pair_)
{
    std::ostringstream Out;
    Out << "\033[";
